...
class FCOSLossComputation(object):
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        cls_loss, lossR, sb_logit = self.cls_loss_func(
            box_cls_flatten,
            labels_flatten.int()
        ) 
        cls_loss = cls_loss / (num_pos_avg_per_gpu + N)  # add N to avoid dividing by a zero
        ...
        if pos_inds.numel() > 0:
            ...
            reg_loss, iou_RD = self.box_reg_loss_func(
                box_regression_flatten,
                reg_targets_flatten,
                centerness_targets
            ) 
            ...
            lossRD = torch.abs(iou_RD - torch.max(sb_logit,1)[0][pos_inds]).mean()
        ...
        return cls_loss + 0.5 * (lossR + 0.1 * lossRD) , reg_loss , centerness_loss
